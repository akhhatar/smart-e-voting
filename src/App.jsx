...
// =========================================================================
const EciLogoHeader = () => (
  <div className="flex items-center justify-center gap-3 md:gap-4 bg-white p-3 rounded-t-xl border-b-[4px] border-[#c0267a] shadow-sm w-full">
    <img src="/eci-logo.png" alt="ECI Logo" className="w-12 h-12 md:w-16 md:h-16 object-contain" onError={(e) => {e.target.src='https://upload.wikimedia.org/wikipedia/commons/thumb/6/6b/Election_Commission_of_India_Logo.svg/1024px-Election_Commission_of_India_Logo.svg.png'}}/>
    <div className="text-left flex flex-col justify-center">
      <h1 className="text-[14px] md:text-[1.3rem] font-bold text-slate-800 leading-tight mb-0.5 tracking-wide">भारत निर्वाचन आयोग</h1>
      <h2 className="text-[10px] md:text-[1rem] font-black text-slate-900 leading-tight tracking-wide uppercase md:normal-case">Election Commission of India</h2>
    </div>
  </div>
);
...
const EciLogoSmall = () => (
  <div className="flex items-center gap-2 bg-white px-2 py-1.5 md:px-3 md:py-1.5 rounded-md border-b-2 border-[#c0267a] shadow-sm">
    ...
    <div className="text-left flex flex-col justify-center">
      <h1 className="text-[9px] md:text-[11px] font-bold text-slate-800 leading-none mb-0.5">भारत निर्वाचन आयोग</h1>
      <h2 className="text-[8px] md:text-[11px] font-black text-slate-900 leading-none tracking-wide">Election Commission of India</h2>
    </div>
  </div>
);
...
const EciAdminSidebarLogo = () => (
  <div className="flex flex-col items-center justify-center text-center">
    ...
    <h1 className="text-[17px] font-black text-[#000080] leading-tight tracking-wide">भारत निर्वाचन आयोग</h1>
    <h2 className="text-[10px] font-bold text-slate-600 mt-1 leading-tight uppercase tracking-wider">Election Commission<br/>of India</h2>
  </div>
);
...
const VotingProvider = ({ children }) => {
  ...
  const [config, setConfig] = useState({ electionStatus: 'upcoming', votes: {}, resultsPublished: false });
  ...
  useEffect(() => {
      ...
      const unsubConfig = onSnapshot(doc(db, publicPath, 'config', 'main'), (docSnap) => {
          ...
          else setDoc(doc(db, publicPath, 'config', 'main'), { electionStatus: 'upcoming', votes: {}, resultsPublished: false });
      });
  ...
  const register = async (userData) => {
      ...
      if (users.some(u => u.aadharNo === userData.aadharNo)) throw new Error("Aadhar Card already registered!");
      if (users.some(u => u.email === userData.email)) throw new Error("Email already registered!");
      if (users.some(u => u.mobile === userData.mobile)) throw new Error("Mobile Number already registered!");
  ...
  const resetElection = async () => {
      if(!db) return;
      try {
          await updateDoc(doc(db, `artifacts/${appId}/public/data`, 'config', 'main'), { electionStatus: 'upcoming', votes: {}, resultsPublished: false });
...
const LoginPage = () => {
  ...
  useEffect(() => {
    loadModels().then((success) => {
        ...
        if(success) setScanStatus("AI Ready. Frame your face.");
  ...
  const handleFaceLogin = async () => {
    if (!modelsLoaded) return setError("Please wait, AI is loading...");
    if (!loginId.trim()) return setError("Kripya pehle apna Aadhar/Mobile number daalein.");
    ...
    try {
      ...
      if (targetUser && targetUser.faceDescriptor) {
        ...
        if (distance < 0.45) { 
          setScanStatus("Match Found! Verifying...");
          try {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center p-4 font-sans relative overflow-hidden transition-all duration-500">
      ...
      {showForgotPass && (
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl border-t-4 border-[#000080] animate-fade-in-up">
                <h3 className="text-xl font-bold text-[#000080] mb-2 flex items-center gap-2"><ShieldCheck size={20}/> Reset Password</h3>
                ...
                {resetSuccess ? (
                    <div className="text-center py-6 animate-fade-in">
                        ...
                        <p className="text-sm text-slate-500">You can now login with your new password.</p>
                    </div>
                ) : !otpSent ? (
                    <form onSubmit={handleSendOTP} className="space-y-4 animate-fade-in">
                        <p className="text-sm text-slate-600 mb-4">Enter your registered email address to receive a secure OTP.</p>
                        <div>
                        ...
                        <div className="flex gap-3 pt-2">
                            ...
                            <button type="submit" className="flex-1 bg-[#000080] text-white py-2.5 rounded-lg font-bold hover:bg-blue-900 transition-colors flex justify-center items-center gap-2"><Mail size={16}/> Send OTP</button>
                        </div>
                    </form>
                ) : (
                    <form onSubmit={handleResetPassword} className="space-y-4 animate-fade-in">
                        <div className="bg-green-50 text-green-700 p-3 rounded-lg text-sm border border-green-200 mb-4">
                            OTP has been sent to <b>{forgotEmail}</b>. Please check your inbox or spam folder.
                        </div>
                        <div>
                        ...
                        <div className="flex gap-3 pt-2">
                            <button type="button" onClick={() => setOtpSent(false)} className="flex-1 bg-slate-200 text-slate-700 py-2.5 rounded-lg font-bold hover:bg-slate-300 transition-colors">Back</button>
                            <button type="submit" disabled={otpInput.length !== 6 || !newPassword} className="flex-1 bg-[#138808] text-white py-2.5 rounded-lg font-bold hover:bg-green-700 transition-colors disabled:bg-slate-300">Verify & Reset</button>
                        </div>
                    </form>
                )}
            </div>
      ...
      <div className="w-full max-w-md text-center mb-6 z-10 animate-fade-in-up">
        <EciLogoHeader />
        <div className="mt-4 py-2 px-6 bg-white border-l-4 border-[#FF9933] border-r-4 border-[#138808] rounded shadow-sm inline-block">
          <h3 className="text-lg font-black text-[#000080]">मतदाता सेवा पोर्टल</h3>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Voters' Service Portal</p>
        </div>
      </div>
      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-slate-200 z-10 animate-fade-in-up delay-100">
        <div className="p-8">
          <h2 className="text-xl font-black text-center text-[#000080] mb-6 border-b pb-3 flex items-center justify-center gap-2">
            <ShieldCheck size={24} className="text-[#138808]"/> SECURE LOGIN
          </h2>
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl mb-6 shadow-inner relative">
            <button onClick={() => {setLoginMethod('password'); setIsScanning(false);}} className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all duration-300 z-10 ${loginMethod === 'password' ? 'bg-white shadow-md text-[#000080]' : 'text-slate-500 hover:text-slate-700'}`}>Password</button>
            <button onClick={() => setLoginMethod('face')} className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all duration-300 z-10 ${loginMethod === 'face' ? 'bg-white shadow-md text-[#000080]' : 'text-slate-500 hover:text-slate-700'}`}>Face Scan</button>
          </div>
          ...
          <form onSubmit={handlePasswordLogin} className="space-y-5">
            <div>
              ...
              <div className="relative">
                 ...
                 <input type="text" required value={loginId} onChange={e => setLoginId(e.target.value)} className="w-full pl-10 p-3.5 bg-slate-50 border border-slate-300 rounded-xl focus:bg-white focus:ring-2 focus:ring-[#000080] outline-none transition-all font-medium" placeholder="Enter your registered ID" />
              </div>
            </div>
            ...
            {loginMethod === 'password' ? (
              <div className="animate-fade-in">
                ...
                <button type="submit" className="w-full mt-4 bg-gradient-to-r from-[#000080] to-blue-900 text-white p-3.5 rounded-xl font-black hover:shadow-lg transform hover:-translate-y-0.5 transition-all tracking-widest uppercase">Login Securely</button>
              </div>
            ) : (
        ...
        <div className="bg-slate-50 p-5 text-center border-t border-slate-200">
          <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mb-2">New Voter Registration?</p>
          <button onClick={() => navigate('/register')} className="text-[#FF9933] text-sm font-black hover:text-orange-600 transition-colors flex items-center justify-center gap-1 mx-auto">Register for e-EPIC <ArrowRight size={16}/></button>
        </div>
      </div>
    </div>
  );
...
const RegisterPage = () => {
  ...
  const startScanning = async () => {
    ...
    setIsScanning(true); setProgress(0); setScanStatus("Keep face steady...");
    ...
    scanIntervalRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (descriptor) {
          ...
          scans++; setProgress(Math.round((scans / requiredScans) * 100)); setScanStatus(`Scanning... ${Math.round((scans / requiredScans) * 100)}%`);
        } else { setScanStatus("Face not detected. Adjust lighting."); }
      } catch (err) { clearInterval(scanIntervalRef.current); setIsScanning(false); setScanStatus("Camera Error!"); }
    }, 300);
  };
  ...
  const handleFinalSubmit = async (e) => {
    ...
    if (users && cleanedFormData.faceDescriptor) {
        const isDuplicateFace = users.some(user => {
            ...
            return calculateDistance(user.faceDescriptor, cleanedFormData.faceDescriptor) < 0.45; 
        });
        if (isDuplicateFace) {
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center p-4 font-sans animate-fade-in relative">
      ...
      {duplicateError && (
        <div className="fixed inset-0 bg-black/70 z-[200] flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-sm p-8 flex flex-col items-center text-center shadow-2xl border-t-8 border-red-600 animate-fade-in-up">
            ...
            <h3 className="text-xl font-black text-slate-800 mb-1 uppercase">Security Warning</h3>
            <p className="text-red-600 font-bold text-xs mb-4 uppercase tracking-widest border-b pb-2">Face Duplication Detected</p>
            <p className="text-slate-600 mb-6 text-sm font-medium">This biometric face profile is already registered with another ID. Creating multiple voter accounts is prohibited.</p>
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-xl overflow-hidden border border-slate-200">
        ...
        <div className="bg-[#000080] p-4 text-white text-center border-b-4 border-[#FF9933]">
          <h2 className="text-xl font-black tracking-widest mb-1 uppercase">Voter Registration</h2>
          ...
          {step === 3 && (
            <div className="space-y-4 animate-fade-in-up">
              <div className="bg-slate-900 rounded-3xl flex flex-col items-center justify-center relative overflow-hidden h-64 border-4 border-[#138808] shadow-inner">
                {formData.facePhoto ? (
                  <div className="text-center w-full h-full relative"><img src={formData.facePhoto} className="w-full h-full object-cover opacity-90" /><div className="absolute inset-0 bg-gradient-to-t from-[#138808]/90 via-transparent flex items-end justify-center pb-6"><p className="text-white font-black flex gap-2"><CheckCircle/> BIOMETRIC SAVED</p></div></div>
                ) : (
                  <>
                    {!modelsLoaded && <div className="absolute inset-0 bg-black/80 z-20 flex items-center justify-center text-white font-bold gap-2"><Loader2 className="animate-spin" size={24}/> Loading Face AI...</div>}
                    <NativeCamera videoRef={videoRef} />
                    ...
                    {!isScanning && <button onClick={startScanning} disabled={!modelsLoaded} className="absolute bottom-6 bg-[#000080] text-white px-6 py-3 rounded-full font-black flex items-center gap-2 border-2 border-white/20 uppercase tracking-wider"><ScanFace size={20}/> Capture Face</button>}
                  </>
                )}
              </div>
              <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                ...
                {formData.facePhoto && <button onClick={()=>{setFormData({...formData, faceDescriptor:null, facePhoto:null}); setProgress(0);}} className="text-xs bg-white border px-3 py-1.5 rounded-lg font-bold flex items-center gap-1"><RefreshCw size={14}/> Retake</button>}
              </div>
              <div className="flex gap-4"><button onClick={() => setStep(2)} className="flex-1 bg-slate-200 p-3.5 rounded-xl font-black uppercase">Back</button><button onClick={handleFinalSubmit} disabled={!formData.facePhoto || isSubmitting} className="flex-1 bg-[#138808] text-white p-3.5 rounded-xl font-black disabled:opacity-50 flex justify-center items-center gap-2 uppercase">{isSubmitting ? <Loader2 className="animate-spin"/> : <ShieldCheck/>} Submit</button></div>
            </div>
          )}
        </div>
        <div className="bg-slate-50 p-4 text-center border-t border-slate-200"><button onClick={() => navigate('/login')} className="text-[#000080] text-sm font-black uppercase">Already have account? Login</button></div>
      </div>
    </div>
  );
};
...
const UserDashboard = () => {
  ...
  if (!currentUser) return <div className="min-h-screen flex items-center justify-center text-[#000080] font-bold"><Loader2 className="animate-spin mr-2"/> Loading Session...</div>;
  ...
  const handleSupportSubmit = async (e) => {
      ...
      sendCustomEmail(currentUser.email, "Support Ticket Raised - ECI", `Dear ${fullName},\n\nWe have received your support ticket regarding "${supportMessage}". Our team will contact you shortly.\n\nRegards,\nECI IT Cell`);
    ...
    <div className="min-h-screen bg-slate-100 font-sans pb-20 md:pb-0">
      ...
      {showVerifyModal && (
        <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-sm p-8 flex flex-col items-center border-t-8 border-[#000080] shadow-2xl animate-fade-in-up">
             <h3 className="text-xl font-black mb-1 text-[#000080] uppercase tracking-wide">Identity Verification</h3>
             ...
             {status === 'failed' && <p className="text-red-600 font-bold bg-red-50 w-full text-center px-4 py-3 rounded-xl border border-red-200">Face Mismatch! <button onClick={()=>setStatus('idle')} className="underline ml-2 text-[#000080]">Retry</button></p>}
             {status === 'success' && <p className="text-[#138808] font-black text-xl flex items-center justify-center gap-2 w-full bg-green-50 px-6 py-3 rounded-xl border border-green-200 uppercase tracking-widest"><CheckCircle/> Verified!</p>}
             {status === 'idle' && <button onClick={()=>setShowVerifyModal(false)} className="mt-4 text-slate-400 hover:text-slate-600 font-bold text-sm uppercase transition-colors">Cancel</button>}
          </div>
        </div>
      )}
      ...
      <nav className="bg-[#000080] shadow-md px-4 py-3 flex justify-between items-center sticky top-0 z-40 border-b-4 border-[#FF9933]">
        ...
        <div className="flex items-center gap-2 bg-blue-900/50 p-1.5 rounded-xl hidden md:flex">
           <button onClick={() => setActiveTab('vote')} className={`px-6 py-2 rounded-lg text-sm font-black uppercase transition-all ${activeTab === 'vote' ? 'bg-white text-[#000080] shadow-md' : 'text-blue-200 hover:bg-white/10'}`}>Cast Vote</button>
           <button onClick={() => setActiveTab('profile')} className={`px-6 py-2 rounded-lg text-sm font-black uppercase transition-all ${activeTab === 'profile' ? 'bg-white text-[#000080] shadow-md' : 'text-blue-200 hover:bg-white/10'}`}>e-EPIC</button>
           <button onClick={() => setActiveTab('support')} className={`px-6 py-2 rounded-lg text-sm font-black uppercase transition-all ${activeTab === 'support' ? 'bg-white text-[#000080] shadow-md' : 'text-blue-200 hover:bg-white/10'}`}>Support</button>
      ...
      {/* Mobile Nav */}
      <div className="md:hidden fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 flex justify-around items-center p-3 pb-safe z-40 shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
         <button onClick={() => setActiveTab('vote')} className={`flex flex-col items-center gap-1 transition-colors ${activeTab === 'vote' ? 'text-[#000080]' : 'text-slate-400'}`}><Fingerprint size={24}/><span className="text-[10px] font-black uppercase">Vote</span></button>
         ...
         <button onClick={() => setActiveTab('settings')} className={`flex flex-col items-center gap-1 transition-colors ${activeTab === 'settings' ? 'text-[#000080]' : 'text-slate-400'}`}><Settings size={24}/><span className="text-[10px] font-black uppercase">Setting</span></button>
      </div>
      ...
      <main className="max-w-5xl mx-auto p-4 md:p-8 mb-20 md:mb-0">
        ...
        {activeTab === 'profile' && (
            <div className="text-center animate-fade-in-up">
                <h2 className="text-3xl font-black text-[#000080] mb-2 uppercase tracking-widest">My Voter Profile</h2>
                <p className="text-slate-500 font-bold mb-6 uppercase text-xs">Digital Electronic Electoral Photo Identity Card</p>
                <VoterIDCard />
                <button onClick={() => window.print()} className="mt-8 bg-[#000080] hover:bg-blue-900 text-white px-8 py-3.5 rounded-xl inline-flex items-center gap-2 font-black shadow-lg transition-transform hover:-translate-y-1 uppercase tracking-widest"><Download size={20}/> Download PDF Receipt</button>
            </div>
        )}
        ...
        {activeTab === 'support' && (
            <div className="max-w-md mx-auto bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden animate-fade-in-up">
                <div className="bg-[#000080] p-6 text-center text-white border-b-4 border-[#FF9933]">
                    ...
                    <h2 className="text-2xl font-black uppercase tracking-widest">Help & Support</h2>
                    <p className="text-blue-200 text-sm font-bold mt-1">Submit a ticket to ECI Admin</p>
                </div>
                {ticketSubmitted ? (
                    <div className="p-8 text-center space-y-4 animate-fade-in">
                        <CheckCircle size={60} className="mx-auto text-[#138808]" />
                        <h3 className="text-xl font-black text-slate-800 uppercase tracking-wide">Ticket Submitted!</h3>
                        <p className="text-slate-500 text-sm font-medium">Our support staff will review your issue and contact you at <b className="text-slate-700">{currentUser.mobile}</b> shortly.</p>
                        <button onClick={() => setTicketSubmitted(false)} className="mt-4 text-[#000080] font-black uppercase tracking-wide underline hover:text-[#FF9933] transition-colors">Submit Another Issue</button>
                    </div>
                ) : (
                    <form onSubmit={handleSupportSubmit} className="p-6 space-y-5 bg-slate-50">
                        <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl flex gap-3 text-sm text-[#000080] font-bold shadow-sm">
                            ...
                            Describe your issue below. ECI admins monitor these tickets continuously.
                        </div>
                        <textarea required value={supportMessage} onChange={(e)=>setSupportMessage(e.target.value)} rows="4" className="w-full p-4 bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#138808] outline-none font-medium resize-none shadow-inner" placeholder="E.g. I am unable to cast my vote due to a camera error..." />
                ...
                {myTickets.length > 0 && (
                    <div className="border-t border-slate-200 p-6 bg-white animate-fade-in">
                        <h3 className="text-sm font-black text-slate-500 uppercase tracking-wider mb-4 text-center">My Previous Tickets</h3>
             ...
             <div className="bg-white p-8 rounded-3xl shadow-sm mb-6 border border-slate-200 border-t-8 border-t-[#FF9933]">
                ...
                <form onSubmit={(e)=>{e.preventDefault(); updateProfile(currentUser.id, { mobile });}}>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <button className="bg-[#000080] hover:bg-blue-900 text-white px-8 py-3.5 font-black rounded-xl shadow-md transition-colors uppercase tracking-widest">Update</button>
                    </div>
                </form>
             </div>
             ...
             <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 border-t-8 border-t-[#138808] mb-6">
                ...
                {passEmailSent ? (
                     <div className="bg-green-50 text-green-700 p-4 rounded-xl font-bold flex items-center gap-2 border border-green-200 animate-fade-in">
                         <CheckCircle size={20}/> Password reset link sent to {currentUser.email}
                     </div>
                ) : (
                    <form onSubmit={handleSendPasswordResetLink}>
                        <div className="mb-5">
                            <label className="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-widest">Registered Email</label>
...
const AdminDashboard = () => {
  ...
  const handlePublishResults = () => {
    if(window.confirm("Toggle Result visibility for public?")) updateConfig({ resultsPublished: !config.resultsPublished });
  };
    ...
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col md:flex-row overflow-hidden relative">
      ...
      {viewImage && (
          <div className="fixed inset-0 bg-black/90 z-[500] flex flex-col items-center justify-center p-4 backdrop-blur-sm animate-fade-in" onClick={() => setViewImage(null)}>
              ...
              <p className="text-white/60 text-sm font-bold mt-4 tracking-widest uppercase">Click anywhere outside to close</p>
          </div>
      )}
      ...
      <aside className="w-full md:w-72 bg-[#000080] text-white flex flex-col shadow-2xl relative z-20 shrink-0 md:h-screen overflow-y-auto">
        ...
        <nav className="flex-1 p-4 space-y-2 mt-4 custom-scrollbar">
          ...
          <button onClick={() => handleTabChange('candidates')} className={`w-full flex items-center gap-3 p-3.5 rounded-xl font-black uppercase tracking-wide transition-all ${activeTab === 'candidates' ? 'bg-white text-[#000080] shadow-md' : 'hover:bg-blue-900/50 text-blue-100'}`}>
              <UserPlus size={20}/> Parties Setup {unseenCandidates > 0 && <span className="ml-auto bg-red-500 text-white px-2 py-0.5 rounded-full text-[10px] animate-pulse">New {unseenCandidates}</span>}
          </button>
          <button onClick={() => handleTabChange('election')} className={`w-full flex items-center gap-3 p-3.5 rounded-xl font-black uppercase tracking-wide transition-all ${activeTab === 'election' ? 'bg-white text-[#000080] shadow-md' : 'hover:bg-blue-900/50 text-blue-100'}`}>
              <Activity size={20}/> Election Phase
          </button>
          <button onClick={() => handleTabChange('tickets')} className={`w-full flex items-center gap-3 p-3.5 rounded-xl font-black uppercase tracking-wide transition-all ${activeTab === 'tickets' ? 'bg-white text-[#000080] shadow-md' : 'hover:bg-blue-900/50 text-blue-100'}`}>
              <Ticket size={20}/> Support Tickets {unseenTickets > 0 && <span className="ml-auto bg-red-500 text-white px-2 py-0.5 rounded-full text-[10px] animate-pulse">New {unseenTickets}</span>}
          </button>
          ...
          <button onClick={() => handleTabChange('logs')} className={`w-full flex items-center gap-3 p-3.5 rounded-xl font-black uppercase tracking-wide transition-all ${activeTab === 'logs' ? 'bg-white text-[#000080] shadow-md' : 'hover:bg-blue-900/50 text-blue-100'}`}>
              <ShieldAlert size={20}/> Security Logs {unseenLogs > 0 && <span className="ml-auto bg-red-500 text-white px-2 py-0.5 rounded-full text-[10px] animate-pulse">New {unseenLogs}</span>}
          </button>
        </nav>
        <div className="p-4 border-t border-blue-900/50"><button onClick={() => {logout(); navigate('/login');}} className="w-full bg-red-600/90 text-white p-3.5 rounded-xl font-black uppercase tracking-wider hover:bg-red-600 flex justify-center items-center gap-2 transition-colors"><LogOut size={18}/> Secure Logout</button></div>
      </aside>
            ...
            <div className="animate-fade-in-up">
              ...
              {voterSubTab === 'pending' ? (
                <div className="space-y-6">
                    {pendingUsers.length === 0 ? <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 shadow-sm"><CheckCircle className="mx-auto w-16 h-16 text-[#138808] opacity-50 mb-3"/><p className="text-lg font-black text-slate-500 uppercase">Queue is empty. Great job!</p></div> : 
            ...
            <div className="animate-fade-in">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[700px]">
                    ...
                    <tbody>
                        {(tickets || []).length === 0 ? (
                           <tr><td colSpan="4" className="p-8 text-center text-slate-500 italic font-medium">No support tickets generated yet.</td></tr>
                        ) : (
                           (tickets || []).map(ticket => (
